...

{
  ...
  programs.bash = {
    ...
    shellAliases = {
      ...
      # update-claudcode
      update-claude = "cd ~/nixos-config && nix flake update claude-code-nix && git commit -am 'chore: update claude-code' && sudo nixos-rebuild switch --flake . || echo 'Update failed'";
    };
    initExtra = ''
      # Starship初期化
